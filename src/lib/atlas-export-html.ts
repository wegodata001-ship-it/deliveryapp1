...
export function atlasExportHtmlStyles(): string {
  ...
  *{box-sizing:border-box}
  ...
  .atlas-pdf-head{
    ...
    gap:16px;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid #e2e8f0;
  }
  .atlas-pdf-head img{height:40px;width:auto;display:block}
  .atlas-pdf-head-text{flex:1;min-width:200px}
  .atlas-brand-en{font-size:11px;font-weight:800;color:#1e3a5f;letter-spacing:.06em;direction:ltr;text-align:right}
  .atlas-report-title{font-size:20px;font-weight:800;color:#0f172a;margin:6px 0 0}
  .atlas-meta-grid{
  ...
  .warn{font-size:12px;color:#b45309;margin-bottom:12px}
  table.atlas-table{width:100%;border-collapse:collapse;font-size:12px;direction:rtl}
  table.atlas-table th,table.atlas-table td{
    border:1px solid #cbd5e1;padding:10px 12px;text-align:right;vertical-align:middle;
  }
  table.atlas-table thead th{
    background:#1e3a5f;color:#fff;font-weight:800;font-size:11px;
  }
  table.atlas-table tbody tr:nth-child(even){background:#f8fafc}
  table.atlas-table tbody tr:hover{background:#f1f5f9}
  ...
  .atlas-footer{
    margin-top:20px;padding:14px 16px;background:#f8fafc;border:1px solid #e2e8f0;
    border-radius:10px;font-size:13px;font-weight:700;display:flex;flex-wrap:wrap;gap:12px 28px;
  }
  .atlas-footer strong{color:#0f172a}
...
export function atlasHtmlHeadBlock(
  ...
  <img src="${ATLAS_PDF_LOGO_DATA_URI}" alt="ATLAS" width="130" height="40" />
...
function atlasHtmlFooterBlock(totals?: AtlasPdfFooterTotals): string {
  if (!totals) return "";
  const parts: string[] = [];
  if (totals.ordersTotalUsd) parts.push(`<span>סה"כ הזמנות: <strong class="num">${escapeHtml(totals.ordersTotalUsd)}</strong></span>`);
  if (totals.paymentsTotalUsd) parts.push(`<span>סה"כ תשלומים: <strong class="num">${escapeHtml(totals.paymentsTotalUsd)}</strong></span>`);
  if (totals.commissionsTotalUsd) parts.push(`<span>סה"כ עמלות: <strong class="num">${escapeHtml(totals.commissionsTotalUsd)}</strong></span>`);
  if (totals.balanceUsd) parts.push(`<span>יתרה: <strong class="num">${escapeHtml(totals.balanceUsd)}</strong></span>`);
