...
import { exportCustomersModuleListAction } from "@/app/admin/customers/actions";
...
import type { CustomerProfileOrderRow, CustomerProfilePaymentRow, CustomersPdfScope } from "@/lib/customers-module-types";
...
import { CustomersPdfScopeModal } from "@/components/admin/CustomersPdfScopeModal";



export type CustomerModuleExportKind = "ledger" | "orders" | "payments" | "open" | "customers";
...
const REPORT_OPTIONS: { value: CustomerModuleExportKind; label: string }[] = [
  ...
  { value: "customers", label: "רשימת לקוחות" },
...
type Props = {
  ...
  /** מדינת עבודה לייצוא רשימת לקוחות */
...
export function CustomerDocumentsPanel({
  ...
  const [customersPdfModalOpen, setCustomersPdfModalOpen] = useState(false);
  ...
  async function runLedgerPdfExport(mode: LedgerPdfMode) {
  ...
  async function runCustomersPdfExport(scope: CustomersPdfScope) {
    ...
    try {
      ...
      const res = await exportCustomersModuleListAction({
      ...
      handleSourceTableExportResult("pdf", res, onToast, downloadBase64File);

      onToast("PDF מוכן לתצוגה");

      setCustomersPdfModalOpen(false);
  ...
  function openPdfFlow() {

    if (busy) return;

    if (reportType === "customers") {

      setCustomersPdfModalOpen(true);
  ...
  async function runExport(format: "pdf" | "excel") {
    ...
    if (format === "pdf" && reportType === "customers") {
    ...
    if (format === "pdf" && (reportType === "ledger" || reportType === "open")) {
    ...
    try {
      ...
      if (reportType === "customers") {
        ...
        const res = await exportCustomersModuleListAction({
          ...
          scope: customerId ? "current" : "all",

          customerId,

          workCountry: workCountry ?? undefined,

          kind: format === "pdf" ? "pdf" : "excel",
      ...
      if (reportType === "ledger" || reportType === "open") {
        ...
        await exportCustomerLedgerExcel(ledgerExportMeta, ledger!);
      ...
      if (reportType === "orders") {
        ...
        onToast(format === "pdf" ? "PDF מוכן לתצוגה" : "Excel הורד בהצלחה");
  ...
  return (
    ...
    <div className={compact ? "adm-cust-docs-card adm-cust-docs-card--compact" : "adm-cust-docs-card"}>
      ...
      {compact ? (
        <label className="adm-cust-docs-card__compact-type">
          <span className="adm-sr-only">סוג מסמך</span>
          <select
            value={reportType}
            disabled={!!busy}
            onChange={(e) => setReportType(e.target.value as CustomerModuleExportKind)}
      ...
      <LedgerPdfExportModal
        ...
        onClose={() => {
          ...
          if (busy !== "pdf") setLedgerPdfModalOpen(false);

        }}

        onExport={(mode) => void runLedgerPdfExport(mode)}

      />

      <CustomersPdfScopeModal

        open={customersPdfModalOpen}

        busy={busy === "pdf"}

        hasCurrentCustomer={!!customerId}

        onClose={() => {

          if (busy !== "pdf") setCustomersPdfModalOpen(false);

        }}

        onExport={(scope) => void runCustomersPdfExport(scope)}
