...
import {
  ...
  listPaymentMethodsManagerAction,
  reorderPaymentMethodsAction,
  updatePaymentMethodAction,
  type PaymentMethodManagerRow,
} from "@/app/admin/payment-methods/actions";
import { usePaymentMethodCatalog } from "@/components/admin/PaymentMethodCatalogProvider";
import { PAYMENT_METHOD_COLOR_PRESETS } from "@/lib/payment-method-shared";
...
function colorLabel(hex: string): string {
  const hit = PAYMENT_METHOD_COLOR_PRESETS.find((p) => p.hex === hex);
...
function ColorPicker({ value, onChange }: { value: string; onChange: (hex: string) => void }) {
  return (
    <div className="adm-status-color-pick adm-status-color-pick--compact">
      {PAYMENT_METHOD_COLOR_PRESETS.map((p) => (
        <button
          key={p.hex}
          type="button"
...
export function PaymentMethodsManager({ initialSearch = "" }: { initialSearch?: string }) {
  const router = useRouter();
  const { refresh: refreshPaymentMethodCatalog } = usePaymentMethodCatalog();
  const [rows, setRows] = useState<PaymentMethodManagerRow[]>([]);
  ...
  const [addColor, setAddColor] = useState<string>(PAYMENT_METHOD_COLOR_PRESETS[0].hex);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await listPaymentMethodsManagerAction(search);
      setRows(data);
  ...
  async function handleCreate() {
    ...
    setAddOpen(false);
    setAddName("");
    setAddColor(PAYMENT_METHOD_COLOR_PRESETS[0].hex);
    setAddActive(true);
    void load();
    refreshPaymentMethodCatalog();
    router.refresh();
  }
  ...
  async function handleSaveEdit() {
    if (!edit) return;
    setBusy(true);
    const res = await updatePaymentMethodAction(edit.id, {
      nameHe: edit.nameHe,
  ...
  async function toggleActive(row: PaymentMethodManagerRow) {
    setBusy(true);
    const res = await updatePaymentMethodAction(row.id, { isActive: !row.isActive });
    setBusy(false);
    if (!res.ok) {
      setErr(res.error);
      return;
  ...
  async function tryDelete() {
    ...
    setBusy(true);
    const res = await deletePaymentMethodAction(deleteState.id, replaceId || undefined);
    setBusy(false);
    if (!res.ok) {
  ...
  async function onDrop(targetId: string) {
    ...
    setDragId(null);
    await reorderPaymentMethodsAction(next);
    void load();
    refreshPaymentMethodCatalog();
    router.refresh();
  }

  async function requestDelete(row: PaymentMethodManagerRow) {
    setBusy(true);
    const res = await deletePaymentMethodAction(row.id);
    setBusy(false);
        ...
        <table className="adm-status-erp-table">
          <thead>
            <tr>
              ...
              <th className="adm-status-erp-col-name">
                <SortBtn col="nameHe" label="שם אמצעי תשלום" />
              </th>
          ...
          <tbody>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="adm-status-erp-empty">
                  אין אמצעי תשלום — לחץ «אמצעי תשלום חדש» להוספה.
                </td>
              </tr>
            ) : (
              displayed.map((row, idx) => (
                ...
                >
                  ...
                  <td className="adm-status-erp-col-name" data-label="שם אמצעי תשלום">
        ...
        <div className="adm-status-pop-overlay" role="presentation" onClick={() => setEdit(null)}>
          <div className="adm-status-pop" role="dialog" aria-labelledby="edit-status-title" onClick={(e) => e.stopPropagation()}>
            <header className="adm-status-pop__head">
              <h2 id="edit-status-title">עריכת אמצעי תשלום</h2>
      ...
      {deleteState ? (
        <div className="adm-status-pop-overlay" role="presentation" onClick={() => setDeleteState(null)}>
          <div className="adm-status-pop adm-status-pop--wide" role="alertdialog" onClick={(e) => e.stopPropagation()}>
            <header className="adm-status-pop__head">
              <h2>האמצעי תשלום בשימוש</h2>
              <button type="button" className="adm-status-pop__close" aria-label="סגור" onClick={() => setDeleteState(null)}>
                <X size={18} />
              </button>
            </header>
            <p>
              «{deleteState.name}» מופיע ב־<strong>{deleteState.usageCount}</strong> הזמנות. העבר להזמנות לאמצעי תשלום אחר לפני מחיקה.
            </p>
            <label className="adm-status-pop__field">
              <span>העבר לאמצעי תשלום</span>
